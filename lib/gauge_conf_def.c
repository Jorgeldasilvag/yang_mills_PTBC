...
void init_gauge_conf_from_file_with_name(Gauge_Conf *GC, GParam const * const param, char const * const filename)
{
	...
	// cold twisted start (only for reduced 12 directions)
	if(param->d_start==3)
	{
		int n,m,cartcoord[STDIM];;
		double complex zf;
		GAUGE_GROUP aux1, aux2, Pmatrix, Qmatrix, aux3;

		one(&aux1);  	
		zero(&aux2);	one(&aux3);
		zero(&Pmatrix); one(&Qmatrix);

		if ((NCOLOR%2)==0) zf = cexp(I*PI/(double)NCOLOR);
		else zf = 1.0 + 0.0*I;

		// P matrix 
		for(n=0; n<(NCOLOR-1); n++)
			aux2.comp[m(n,n+1)] = zf;
		aux2.comp[m(2,0)] = zf;
		for(n=0; n<NCOLOR; n++)
			for(m=0; m<NCOLOR; m++)
				Pmatrix.comp[m(n,m)] = conj(aux2.comp[m(m,n)]);

		// Q matrix
		for(n=0; n<NCOLOR; n++)
			Qmatrix.comp[m(n,n)] = zf*cexp(PI2*I*(n+1)/(double)NCOLOR);
		for(n=0; n<param->d_k_twist[3]; n++) 
			times_equal(&aux3,&Qmatrix);
		equal(&Qmatrix,&aux3);
		...
		for(r=0; r<(param->d_volume); r++)
		{
			equal(&(GC->lattice[r][0]), &aux1);
			equal(&(GC->lattice[r][3]), &aux1);
			si_to_cart(cartcoord, r, param);
			if (cartcoord[1] == 0) equal(&(GC->lattice[r][1]), &Pmatrix);
			else equal(&(GC->lattice[r][1]), &aux1);
			if (cartcoord[2] == 0) equal(&(GC->lattice[r][2]), &Qmatrix);
			else equal(&(GC->lattice[r][2]), &aux1);
		}

		for(r=0; r<(param->d_volume); r++)
			for(j=0; j<STDIM; j++)
				unitarize(&(GC->lattice[r][j]));
	}
}
...
{
	long r;
	int err,mu,nu,cartcoord[STDIM];
	
	err=posix_memalign((void**) &(GC->ztw), (size_t) DOUBLE_ALIGN, (size_t) param->d_volume * sizeof(complex double *));
	if(err!=0)
	{
	...
	for(r=0; r<(param->d_volume); r++)
	{
		err=posix_memalign((void**) &(GC->ztw[r]), (size_t) DOUBLE_ALIGN, (size_t) param->d_n_planes * sizeof(complex double));
		if(err!=0)
		{
			fprintf(stderr, "Problems in allocating the twist factors! (%s, %d)\n", __FILE__, __LINE__);
			exit(EXIT_FAILURE);
	...
	// initialization of ztw[r][mu]

	for(r=0; r<param->d_volume; r++)
		for(mu=0; mu<param->d_n_planes; mu++)
				GC->ztw[r][mu]=1.0+0.0*I;
...
void write_conf_on_file_with_name(Gauge_Conf const * const GC,
	...
	for(lex=0; lex<param->d_volume; lex++)
		...
		for(mu=0; mu<STDIM; mu++)
            ...
            //  for(j=0; j<NCOLOR; j++)
            //  {
            //   printf("%f %f %d %d %d %d %d %d %d\n",creal(GC->lattice[si][mu].comp[m(i,j)]),cimag(GC->lattice[si][mu].comp[m(i,j)]),mu,i,j,cart[0],cart[1],cart[2],cart[3]);
            //  }
            //}
			}
		}
